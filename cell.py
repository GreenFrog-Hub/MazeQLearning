...
class cell:
    def __init__(self, cellType,x, y):
        ...
        if self.cellType == "O":
            ...
            self.cellObj = pg.sprite.Sprite(img=self.spikeImg, x= x*52, y=572-(y*52))
        else:
            self.cellObj = pg.shapes.BorderedRectangle(x*52, 572-(y*52), 52, 52, 1, color=self.colour, border_color=(0,0,0,255))
